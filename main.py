...
if 'state' not in st.session_state:
    ...
    def login():
        # verify password
        if st.session_state['password'] == 'gaur':
            st.session_state['state'] = 'feed'
    loginbutton = st.button("Login",on_click=login)

#-----------------------------feed page
elif st.session_state['state'] == 'feed':
    
    st.header(':green[Barasana Dhaam]')
    st.subheader(':blue[Sri Sri Nimai Nitai Ki Jai]')
    st.image('nn.jpg',width=500)

    st.markdown('---') #------------------------ Forms
    
    st.subheader(":blue[Quick Forms]")    
    st.button('Sadhana Card')
